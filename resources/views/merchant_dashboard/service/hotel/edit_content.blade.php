...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item" aria-current="page"><a href="{{ route('service_listing',$data->description) }}"> {{ $data->name }}</a></li>
...
<div class="card-body">
<form action="{{ route('m_update_hotel_content',$data->content_id) }}" method="post"> 
@csrf       
<div class="form-group">
  ...
  <select class="custom-select" name="hotel" id="hotel">
    <!-- <option value="{{ $data->ps_id }}" class="text-muted">{{ $data->ps_name }} - Address: {{ $data->ps_address }}</option> -->
    @forelse($hotel as $hotels)
        <option value="{{ $hotels->ps_id }}" {{ old('hotel',$data->serviceid) == $hotels->ps_id ? 'selected':'' }}>{{ $hotels->ps_name }} - Address: {{ $hotels->ps_address }}</option>
      @empty
...
<div class="form-group">
  ...
  <input type="text" name="room_name" value="{{ old('room_name',$data->tour_name) }}" class="form-control" placeholder="Room Name">
</div>
...
<div class="form-group">
  ...
  <textarea name="room_description" class="form-control" rows="2" id="description-textarea" placeholder="Room Description ...">{{ old('room_description',$data->roomdesc) }}</textarea>
</div>
...
<div class="form-group">
  ...
  <select class="custom-select facilities" name="room_facilities[]" multiple="multiple">
      ...
      @foreach(explode(',',$data->room_facilities) as $list)
        @foreach($room_amenities as $lists)
            <option value="{{ $lists->name }}" {{ old('room_facilities',$list) == $lists->name ? 'selected':'' }}>{{ $lists->name }} 
        @endforeach
      @endforeach
  </select>
</div>
...
<div class="row">
  ...
  <input type="text" name="price" value="{{ old('price',$data->price) }}" id="currency-field" data-type="currency" class="form-control" placeholder="Price" data-rule="minlen:5">
  </div>
...
<input type="text" name="no_night" value="{{ old('no_night',$data->nonight) }}" class="form-control" placeholder="Number of Night">
</div>
</div>
...
<input type="text" name="no_guest" value="{{ old('no_guest',$data->noguest) }}" class="form-control" placeholder="Number of Guest">
</div>
...
<input type="text" name="quantity" value="{{ old('quantity',$data->qty) }}" class="form-control" placeholder="Quantity">
</div>
</div>
...
<div class="row">
  ...
  <input type="text" name="room_size" value="{{ old('room_size',$data->roomsize) }}"  class="form-control" placeholder="Room Size">
  </div>
...
<div class="form-group col-4">
  ...
  <select class="custom-select" name="views">
      ...
      <option value="City View" {{ old('views',$data->viewdeck) == 'City View' ? 'selected':'' }}>City View</option>
      <option value="Forest View" {{ old('views',$data->viewdeck) == 'Forest View' ? 'selected':'' }}>Forest View</option>
      <option value="Seaside View" {{ old('views',$data->viewdeck) == 'Seaside View' ? 'selected':'' }}>Seaside View</option>
...
<input type="text" name="number_bed" value="{{ old('number_bed',$data->nobed) }}" class="form-control" placeholder="Number of Bed">
</div>
</div>
...
<div class="card-footer">
  <button type="submit" class="btn btn-info">Save <i class="fas fa-chevron-circle-right"></i></button>
</div>
