...
protected function get_service_name($category, $limit=null) {
    ...
    $service_name = TourModel::join('locations_district','locations_district.id', 'service_tour.district')
                ...
                ->join('products','service_tour.service_id', 'products.id')
                    ->where([['service_tour.temp_status',1],['products.description',$category]
...
protected function get_service_exclusive($exclusive, $limit=null) {
    
    $service_exclusive = TourModel::join('locations_district','locations_district.id', 'service_tour.district')
                ->join('location_country','location_country.id', 'locations_district.country_id')
                ->join('products','service_tour.service_id', 'products.id')
                    ->where([
                                ['service_tour.temp_status',1],['products.description',$exclusive]
                            ])->inRandomOrder()->limit($limit)
                         ->get(['service_tour.id as serviceid','service_tour.cover',
                            'service_tour.tour_name',
                            'service_tour.price',
        ...
        else 
        {
                ...
                return $service_exclusive;
            }

    }


protected function get_service_rooms($limit=null) {
    
    $service_rooms = TourModel::join('locations_district','locations_district.id', 'service_tour.district')
                ->join('location_country','location_country.id', 'locations_district.country_id')
                ->join('products','service_tour.service_id', 'products.id')
                    ->where([
                                ['service_tour.temp_status',1],['products.description','hotel_and_resort']
                            ])->inRandomOrder()->limit($limit)
                         ->get(['service_tour.id as serviceid','service_tour.cover',
                            'service_tour.tour_name',
                            'service_tour.price',
...
protected function get_service_tour($limit=null) {
    
    $service_tours = TourModel::join('locations_district','locations_district.id', 'service_tour.district')
                ->join('location_country','location_country.id', 'locations_district.country_id')
                ->join('products','service_tour.service_id', 'products.id')
                    ->where([
                                ['service_tour.temp_status',1],['products.description','tour_operator']
                            ])->inRandomOrder()->limit($limit)
                         ->get(['service_tour.id as serviceid','service_tour.cover',
                            'service_tour.tour_name',
                            'service_tour.price',
